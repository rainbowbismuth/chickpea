		...
		if (isgray(thread) && thread->openupval != NULL)
			...
			*p = thread->twups; /* remove thread from the list */
			thread->twups = thread; /* mark that it is out of list
						 */
			for (uv = thread->openupval; uv != NULL;
			     uv = uv->u.open.next) {
				work++;
...
static void traverseweakvalue(global_State *g, Table *h)
{
	...
	for (n = gnode(h, 0); n < limit; n++) { /* traverse hash part */
		if (isempty(gval(n))) /* entry is empty? */
			...
			markkey(g, n);
			if (!hasclears && iscleared(g, gcvalueN(gval(n)))) /* a
									      white
									      value?
									    */
...
static int traversethread(global_State *g, lua_State *th)
{
	...
	lua_assert(g->gcstate == GCSatomic || th->openupval == NULL
		   || isintwups(th));
...
static void convergeephemerons(global_State *g)
{
	...
	do {
		...
		g->ephemeron = NULL; /* tables may return to this list when
					traversed */
		changed = 0;
		while ((w = next) != NULL) { /* for each ephemeron table */
			next = gco2t(w)->gclist; /* list is rebuilt during loop
						  */
			if (traverseephemeron(g, gco2t(w), dir)) { /* marked
								      some
								      value? */
				propagateall(g); /* propagate changes */
				changed = 1; /* will have to revisit all
						ephemeron tables */
			}
		}
...
static void checkSizes(lua_State *L, global_State *g)
{
	if (!g->gcemergency) {
		if (g->strt.nuse < g->strt.size / 4) { /* string table too big?
			...
			g->GCestimate += g->GCdebt - olddebt; /* correct
								 estimate */
		}
	}
}
...
static void GCTM(lua_State *L)
{
	...
	if (!notm(tm)) { /* is there a finalizer? */
		...
		status = luaD_pcall(L, dothecall, NULL,
				    savestack(L, L->top - 2), 0);
...
static void separatetobefnz(global_State *g, int all)
{
	...
	while ((curr = *p) != g->finobjold) { /* traverse all finalizable
						 objects */
		...
		if (!(iswhite(curr) || all)) /* not being collected? */
			...
			*p = curr->next; /* remove 'curr' from 'finobj' list */
			curr->next = *lastnext; /* link at the end of 'tobefnz'
						   list */
			*lastnext = curr;
			lastnext = &curr->next;
		}
...
void luaC_checkfinalizer(lua_State *L, GCObject *o, Table *mt)
{
	...
	if (tofinalize(o) || /* obj. is already marked... */
		...
		if (issweepphase(g)) {
			...
			if (g->sweepgc == &o->next) /* should not remove
						       'sweepgc' object */
				g->sweepgc = sweeptolive(L, g->sweepgc); /* change
									    'sweepgc'
									  */
		...
		if (iswhite(curr)) { /* is 'curr' dead? */
			...
			*p = curr->next; /* remove 'curr' from list */
			...
			if (getage(curr) == G_NEW)
				curr->marked = cast_byte(
					(curr->marked & maskgencolors) | white);
...
static GCObject **correctgraylist(GCObject **p)
{
	...
	while ((curr = *p) != NULL) {
		...
		case LUA_VUSERDATA: {
			...
			if (getage(curr) == G_TOUCHED1) { /* touched in this
							     cycle? */
				lua_assert(isgray(curr));
				gray2black(curr); /* make it black, for next
						     barrier */
				...
				if (!iswhite(curr)) { /* not white? */
					...
					if (getage(curr) == G_TOUCHED2) /* advance
									   from
									   G_TOUCHED2...
									 */
						changeage(curr, G_TOUCHED2,
							  G_OLD); /* ... to
								     G_OLD */
					gray2black(curr); /* make it black */
				}
				/* else, object is white: just remove it from
				 * this list */
...
static void markold(global_State *g, GCObject *from, GCObject *to)
{
	...
	for (p = from; p != to; p = p->next) {
		if (getage(p) == G_OLD1) {
			...
			if (isblack(p)) {
				black2gray(p); /* should be '2white', but gray
						  works too */
...
{
	luaE_setdebt(g,
		     -(cast(l_mem, (gettotalbytes(g) / 100)) * g->genminormul));
}
...
static void genstep(lua_State *L, global_State *g)
{
	if (g->lastatomic != 0) /* last collection was a bad one? */
		...
		lu_mem majorbase = g->GCestimate; /* memory after last major
						     collection */
		lu_mem majorinc =
			(majorbase / 100) * getgcparam(g->genmajormul);
		if (g->GCdebt > 0 && gettotalbytes(g) > majorbase + majorinc) {
			lu_mem numobjs = fullgen(L, g); /* do a major collection
							 */
			if (gettotalbytes(g) < majorbase + (majorinc / 2)) {
				/* collected at least half of memory growth
				   since last major
				   collection; keep doing minor collections */
				setminordebt(g);
			} else { /* bad collection */
				g->lastatomic = numobjs; /* signal that last
							    collection was bad
							  */
				setpause(g); /* do a long wait for next (major)
						collection */
			}
...
static lu_mem singlestep(lua_State *L)
{
	...
	case GCSenteratomic: {
		lu_mem work = atomic(L); /* work is what was traversed by
					    'atomic' */
		entersweep(L);
		g->GCestimate = gettotalbytes(g); /* first estimate */
		;
...
static void incstep(lua_State *L, global_State *g)
{
	...
	l_mem stepsize = (g->gcstepsize <= log2maxs(l_mem))
			       ? ((cast(l_mem, 1) << g->gcstepsize) / WORK2MEM)
					 * stepmul
	...
	if (g->gcstate == GCSpause)
		...
		debt = (debt / stepmul) * WORK2MEM; /* convert 'work units' to
						       bytes */
		luaE_setdebt(g, debt);
	}
}
