...
static unsigned int luai_makeseed(lua_State *L __attribute__((unused)))
{
...
LUA_API int lua_setcstacklimit(lua_State *L, unsigned int limit)
{
	...
	if (L != g->mainthread)
		...
		return limit - diff - CSTACKERR; /* success; return previous
						    limit */
	}
}
...
void luaE_enterCcall(lua_State *L)
{
	...
	if (ncalls <= CSTACKERR) { /* possible overflow? */
		...
		if (ncalls <= CSTACKERR) { /* still overflow? */
			if (ncalls <= CSTACKERRMARK) /* below error-handling
							zone? */
				luaD_throw(L, LUA_ERRERR); /* error while
							      handling stack
							      error */
			else if (ncalls >= CSTACKMARK) {
				/* not in error-handling zone; raise the error
				 * now */
				L->nCcalls = (CSTACKMARK - 1); /* enter
								  error-handling
								  zone */
...
void luaE_warnerror(lua_State *L, const char *where)
{
	...
	const char *msg = (ttisstring(errobj)) ? svalue(errobj)
