	...
	*endptr = cast_charp(s); /* nothing is valid yet */
	...
	for (s += 2;; s++) { /* skip '0x' and read numeral */
		if (*s == dot) {
			...
			if (sigdig == 0 && *s == '0') /* non-significant digit
							 (zero)? */
				nosigdig++;
			else if (++sigdig <= MAXSIGDIG) /* can read it without
							   overflow? */
				r = (r * cast_num(16.0)) + luaO_hexavalue(*s);
			else
				e++; /* too many digits; ignore, but still count
					for exponent */
			if (hasdot)
...
static const char *l_str2dloc(const char *s __attribute__((unused)),
			      lua_Number *result __attribute__((unused)),
			      int mode __attribute__((unused)))
{
	return NULL;
	//  char *endptr;
	//  *result = (mode == 'x') ? lua_strx2number(s, &endptr)  /* try to
	//  convert */
	//                          : lua_str2number(s, &endptr);
	//  if (endptr == s) return NULL;  /* nothing recognized? */
	//  while (lisspace(cast_uchar(*endptr))) endptr++;  /* skip trailing
	//  spaces */ return (*endptr == '\0') ? endptr : NULL;  /* OK if no
	//  trailing characters */
}
...
static const char *l_str2d(const char *s, lua_Number *result)
{
	...
	if (endptr == NULL) { /* failed? may be a different locale */
		...
		buff[pdot - s] = lua_getlocaledecpoint(); /* correct decimal
							     point */
...
static const char *l_str2int(const char *s, lua_Integer *result)
{
	...
	if (s[0] == '0' && (s[1] == 'x' || s[1] == 'X')) { /* hex? */
		...
		for (; lisdigit(cast_uchar(*s)); s++) {
			int d = *s - '0';
			if (a >= MAXBY10
			    && (a > MAXBY10 || d > MAXLASTD + neg)) /* overflow?
								     */
...
int luaO_utf8esc(char *buff, unsigned long x)
{
	...
	if (x < 0x80) /* ascii? */
		...
		do { /* add continuation bytes */
			...
			mfb >>= 1; /* now there is one less bit available in
				      first byte */
		} while (x > mfb); /* still needs continuation byte? */
		buff[UTF8BUFFSZ - n] = cast_char((~mfb << 1) | x); /* add first
								      byte */
	}
	return n;
}
...
static int tostringbuff(TValue *obj, char *buff)
{
	...
	if (ttisinteger(obj))
		...
		if (buff[strspn(buff, "-0123456789")] == '\0') { /* looks like
								    an int? */
...
static void pushstr(BuffFS *buff, const char *str, size_t l)
{
	...
	if (buff->pushed > 1 && L->top + 1 >= L->stack_last) {
		luaV_concat(L, buff->pushed); /* join all partial results into
						 one */
		buff->pushed = 1;
	}
}
...
const char *luaO_pushvfstring(lua_State *L, const char *fmt, va_list argp)
{
	...
	while ((e = strchr(fmt, '%')) != NULL) {
		...
		case 'I': { /* a 'lua_Integer' */
			TValue num;
			setivalue(&num,
				  cast(lua_Integer, va_arg(argp, l_uacInt)));
			addnum2buff(&buff, &num);
			break;
		}
		case 'f': { /* a 'lua_Number' */
		...
		case 'p': { /* a pointer */
			const int sz = 3 * sizeof(void *) + 8; /* enough space
								  for '%p' */
			char *bf __attribute__((unused)) = getbuff(&buff, sz);
			void *p __attribute__((unused)) = va_arg(argp, void *);
...
void luaO_chunkid(char *out, const char *source, size_t srclen)
{
	...
	if (*source == '=') { /* 'literal' source */
		...
		if (srclen <= bufflen) /* small enough? */
			...
			bufflen -= LL(RETS);
			memcpy(out, source + 1 + srclen - bufflen,
			       bufflen * sizeof(char));
		}
		...
		bufflen -= LL(PRE RETS POS) + 1; /* save space for
						    prefix+suffix+'\0' */
