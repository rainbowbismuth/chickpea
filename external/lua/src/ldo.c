...
void luaD_seterrorobj(lua_State *L, int errcode, StkId oldtop)
{
	...
	case LUA_ERRMEM: { /* memory error? */
		setsvalue2s(L, oldtop, G(L)->memerrmsg); /* reuse preregistered
							    msg. */
		break;
	}
	case LUA_ERRERR: {
	...
	default: {
		setobjs2s(L, oldtop, L->top - 1); /* error message on current
						     top */
		break;
	}
	}
...
l_noret luaD_throw(lua_State *L, int errcode)
{
	if (L->errorJmp) { /* thread has an error handler? */
		...
		global_State *g = G(L);
		errcode = luaF_close(L, L->stack, errcode); /* close all
							       upvalues */
		L->status = cast_byte(errcode); /* mark it as dead */
		if (g->mainthread->errorJmp) { /* main thread has a handler? */
			setobjs2s(L, g->mainthread->top++, L->top - 1); /* copy
									   error
									   obj.
									 */
			luaD_throw(g->mainthread, errcode); /* re-throw in main
							       thread */
		} else { /* no handler at all; abort */
			if (g->panic) { /* panic function? */
				luaD_seterrorobj(L, errcode, L->top); /* assume
									 EXTRA_STACK
								       */
				if (L->ci->top < L->top)
					L->ci->top = L->top; /* pushing msg. can
								break this
								invariant */
				lua_unlock(L);
				g->panic(L); /* call panic function (last chance
						to jump out) */
			}
			abort();
		}
...
static void correctstack(lua_State *L, StkId oldstack, StkId newstack)
{
	...
	for (ci = L->ci; ci != NULL; ci = ci->previous) {
		...
		if (isLua(ci))
			ci->u.l.trap = 1; /* signal to update 'trap' in
					     'luaV_execute' */
	}
}
...
int luaD_reallocstack(lua_State *L, int newsize, int raiseerror)
{
	int lim = L->stacksize;
	StkId newstack =
		luaM_reallocvector(L, L->stack, lim, newsize, StackValue);
...
int luaD_growstack(lua_State *L, int n, int raiseerror)
{
	...
	if (unlikely(size > LUAI_MAXSTACK)) { /* need more space after extra
						 size? */
		if (raiseerror)
			luaD_throw(L, LUA_ERRERR); /* error inside message
						      handler */
		else
			return 0;
	} else {
...
{
	int hook = (ci->callstatus & CIST_TAIL) ? LUA_HOOKTAILCALL
						: LUA_HOOKCALL;
	Proto *p;
...
static void moveresults(lua_State *L, StkId res, int nres, int wanted)
{
	...
	case 1: /* one value needed */
		if (nres == 0) /* no results? */
			...
			setobjs2s(L, res, L->top - nres); /* move it to proper
							     place */
		L->top = res + 1;
		return;
	case LUA_MULTRET:
...
retry:
	...
	case LUA_VLCL: { /* Lua function */
		CallInfo *ci = next_ci(L);
		Proto *p = clLvalue(s2v(func))->p;
		int narg = cast_int(L->top - func) - 1; /* number of real
							   arguments */
		int nfixparams = p->numparams;
		int fsize = p->maxstacksize; /* frame size */
		...
		for (; narg < nfixparams; narg++)
			setnilvalue(s2v(L->top++)); /* complete missing
						       arguments */
		lua_assert(ci->top <= L->stack_last);
...
static void finishCcall(lua_State *L, int status)
{
	...
	if (ci->callstatus & CIST_YPCALL) { /* was inside a pcall? */
		ci->callstatus &= ~CIST_YPCALL; /* continuation is also inside
						   it */
		L->errfunc = ci->u.c.old_errfunc; /* with the same error
						     function */
	}
...
static void unroll(lua_State *L, void *ud)
{
	...
	while ((ci = L->ci) != &L->base_ci) { /* something in the stack */
		if (!isLua(ci)) /* C function? */
			...
			luaV_execute(L, ci); /* execute down to higher C
						'boundary' */
		}
	}
}
...
LUA_API int lua_resume(lua_State *L, lua_State *from, int nargs, int *nresults)
{
	...
	if (L->status == LUA_OK) { /* may be starting a coroutine */
		if (L->ci != &L->base_ci) /* not in base level? */
			return resume_error(
				L, "cannot resume non-suspended coroutine",
				nargs);
		       ...
		       lua_KFunction k)
{
	...
	if (unlikely(!yieldable(L))) {
		if (L != G(L)->mainthread)
			luaG_runerror(
				L, "attempt to yield across a C-call boundary");
		else
			luaG_runerror(
				L, "attempt to yield from outside a coroutine");
	}
	L->status = LUA_YIELD;
	       ...
	       ptrdiff_t ef)
{
	...
	if (unlikely(status != LUA_OK)) { /* an error occurred? */
		...
		oldtop = restorestack(L, old_top); /* previous call may change
						      stack */
		luaD_seterrorobj(L, status, oldtop);
		luaD_shrinkstack(L);
	}
...
static void checkmode(lua_State *L, const char *mode, const char *x)
{
	if (mode && strchr(mode, x[0]) == NULL) {
		luaO_pushfstring(L, "attempt to load a %s chunk (mode is '%s')",
				 x, mode);
