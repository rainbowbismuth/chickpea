...
static TValue *index2value(lua_State *L, int idx)
{
	...
	if (idx > 0) {
		StkId o = ci->func + idx;
		api_check(L, idx <= L->ci->top - (ci->func + 1),
			  "unacceptable index");
		if (o >= L->top)
			return &G(L)->nilvalue;
		else
...
LUA_API void lua_arith(lua_State *L, int op)
{
	...
	if (op != LUA_OPUNM && op != LUA_OPBNOT)
		api_checknelems(L, 2); /* all other operations expect two
					  operands */
...
LUA_API const char *lua_tolstring(lua_State *L, int idx, size_t *len)
{
	...
	if (!ttisstring(o)) {
		...
		luaO_tostring(L, o);
		luaC_checkGC(L);
		o = index2value(L, idx); /* previous call may reallocate the
					    stack */
		lua_unlock(L);
	}
	if (len != NULL)
		       ...
		       lua_KContext ctx, lua_KFunction k)
{
	...
	if (errfunc == 0)
		...
		api_check(L, ttisfunction(s2v(o)),
	...
	if (k == NULL || !yieldable(L)) { /* no continuation or no yieldable? */
		...
		L->errfunc = func;
		setoah(ci->callstatus, L->allowhook); /* save value of
							 'allowhook' */
		     ...
		     const char *chunkname, const char *mode)
{
	...
	if (status == LUA_OK) { /* no errors? */
		LClosure *f = clLvalue(s2v(L->top - 1)); /* get newly created
							    function */
		if (f->nupvalues >= 1) { /* does it have an upvalue? */
			/* get global table from registry */
			Table *reg = hvalue(&G(L)->l_registry);
			const TValue *gt = luaH_getint(reg, LUA_RIDX_GLOBALS);
			/* set global table as 1st upvalue of 'f' (may be
			 * LUA_ENV) */
