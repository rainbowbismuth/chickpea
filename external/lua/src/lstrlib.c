...
static int str_rep(lua_State *L)
{
	...
	if (n <= 0)
		...
		memcpy(p, s, l * sizeof(char)); /* last copy (not followed by
						   separator) */
...
static const luaL_Reg stringmetamethods[] = { { "__index",
						NULL }, /* placeholder
							 */
					      { NULL, NULL } };
...
typedef struct MatchState {
	...
	lua_State *L;
	int matchdepth; /* control for recursive depth (to avoid C stack
			   overflow) */
	unsigned char level; /* total number of captures (finished or
				unfinished) */
	struct {
		const char *init;
		ptrdiff_t len;
...
static int num2straux(char *buff, int sz, lua_Number x)
{
	...
	if (x != x || x == (lua_Number)HUGE_VAL || x == -(lua_Number)HUGE_VAL)
		...
		return l_sprintf(buff, sz, LUA_NUMBER_FMT "x0p+0",
				 (LUAI_UACNUMBER)x);
	} else {
		int e;
		lua_Number m = l_mathop(frexp)(x, &e); /* 'x' fraction and
							  exponent */
			   ...
			   lua_Number x)
{
	...
	if (fmt[SIZELENMOD] == 'A') {
		...
		return luaL_error(
			L, "modifiers for format '%%a'/'%%A' not implemented");
	return n;
}
...
//      */ int nb = 0;  /* number of bytes in added item */ if (++arg > top)
...
static int getnumlimit(Header *h, const char **fmt, int df)
{
	...
	if (sz > MAXINTSIZE || sz <= 0)
		return luaL_error(h->L,
				  "integral size (%d) out of limits [1,%d]", sz,
				  MAXINTSIZE);
	return sz;
}
			  ...
			  int *psize, int *ntoalign)
{
	...
	if (opt == Kpaddalign) { /* 'X' gets alignment from following option */
		if (**fmt == '\0' || getoption(h, fmt, &align) == Kchar
		    || align == 0)
			luaL_argerror(h->L, 1,
	...
	while (*fmt != '\0') {
		...
		case Kfloat: { /* floating-point options */
			...
			if (size == sizeof(u.f))
				...
				u.n = n;
			/* move 'u' to final result, correcting endianness if
			 * needed */
		...
		case Kstring: { /* strings with length count */
			...
			packint(&b, (lua_Unsigned)len, h.islittle, size,
				0); /* pack length */
			     ...
			     int size, int issigned)
{
	...
	if (size < SZINT) { /* real size smaller than lua_Integer? */
		...
		for (i = limit; i < size; i++) {
			if ((unsigned char)str[islittle ? i : size - 1 - i]
			    != mask)
				luaL_error(
					L,
