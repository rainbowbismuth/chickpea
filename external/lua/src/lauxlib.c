		...
		if (lua_type(L, -2) == LUA_TSTRING) { /* ignore non-string keys
			...
			if (lua_rawequal(L, objidx, -1)) { /* found object? */
				...
				lua_pushliteral(L, "."); /* place '.' between
							    the two names */
				lua_replace(L, -3); /* (in the slot occupied by
						       table) */
...
static int pushglobalfuncname(lua_State *L, lua_Debug *ar)
{
	...
	if (findfield(L, top + 1, 2)) {
		...
		if (strncmp(name, LUA_GNAME ".", 3) == 0) { /* name start with
							       '_G.'? */
...
static void pushfuncname(lua_State *L, lua_Debug *ar)
{
	if (pushglobalfuncname(L, ar)) { /* try first a global name */
		...
		lua_pushfstring(L, "function <%s:%d>", ar->short_src,
				ar->linedefined);
	else /* nothing left... */
		lua_pushliteral(L, "?");
}
			       ...
			       int level)
{
	...
	while (lua_getstack(L1, level++, &ar)) {
		if (limit2show-- == 0) { /* too many levels? */
			int n = last - level - LEVELS2 + 1; /* number of levels
							       to skip */
			lua_pushfstring(L, "\n\t...\t(skipping %d levels)", n);
	...
	if (p != NULL) { /* value is a userdata? */
		if (lua_getmetatable(L, ud)) { /* does it have a metatable? */
			...
			if (!lua_rawequal(L, -1, -2)) /* not the same? */
				p = NULL; /* value is a userdata with wrong
					     metatable */
...
static char *prepbuffsize(luaL_Buffer *B, size_t sz, int boxidx)
{
	if (B->size - B->n >= sz) /* enough space? */
		...
		if (buffonstack(B)) /* buffer already has a box? */
			...
			memcpy(newbuff, B->b, B->n * sizeof(char)); /* copy
								       original
								       content
								     */
		}
...
static const char *getF(lua_State *L, void *ud, size_t *size)
{
	...
	if (lf->n > 0) { /* are there pre-read characters to be read? */
		*size = lf->n; /* return them (chars already in buffer) */
		...
		/* 'fread' can return > 0 *and* set the EOF flag. If next call
		   to 'getF' called 'fread', it might still wait for user input.
		   The next check avoids this problem. */
		if (feof(lf->f))
			return NULL;
		*size = fread(lf->buff, 1, sizeof(lf->buff), lf->f); /* read
									block */
	}
	return lf->buff;
}
	...
	if (luaL_callmeta(L, idx, "__tostring")) { /* metafield? */
		...
		case LUA_TNUMBER: {
			if (lua_isinteger(L, idx))
				...
				lua_pushfstring(
					L, "%f",
					(LUAI_UACNUMBER)lua_tonumber(L, idx));
			break;
		}
		case LUA_TSTRING:
			lua_pushvalue(L, idx);
			break;
		case LUA_TBOOLEAN:
			lua_pushstring(L, (lua_toboolean(L, idx) ? "true"
								 : "false"));
			break;
		case LUA_TNIL:
			...
			break;
		default: {
			int tt = luaL_getmetafield(L, idx, "__name"); /* try
									 name */
			const char *kind = (tt == LUA_TSTRING)
						 ? lua_tostring(L, -1)
						 : luaL_typename(L, idx);
...
static void warnf(void *ud, const char *message, int tocont)
{
	...
	if (*warnstate == 1) /* previous message was the last? */
		lua_writestringerror("%s", "Lua warning: "); /* start a new
								warning */
