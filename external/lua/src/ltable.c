...
#endif
		size++;
		lua_assert(ispow2(size) && size / 2 < t->alimit
			   && t->alimit < size);
		return size;
	}
}
			      ...
			      unsigned int asize)
{
	...
	if (i - 1u < asize) /* is 'key' inside array part? */
		...
		if (unlikely(isabstkey(n)))
			luaG_runerror(L, "invalid key to 'next'"); /* key not
								      found */
		i = cast_int(nodefromval(n) - gnode(t, 0)); /* key index in hash
							       table */
...
static unsigned int computesizes(unsigned int nums[], unsigned int *pna)
{
	...
	for (i = 0, twotoi = 1; twotoi > 0 && *pna > twotoi / 2;
	     i++, twotoi *= 2) {
		a += nums[i];
		if (a > twotoi / 2) { /* more than half elements present? */
			optimal = twotoi; /* optimal size (till now) */
			na = a; /* all elements up to 'optimal' will go to array
				   part */
		}
	}
...
const TValue *luaH_get(Table *t, const TValue *key)
{
	...
	case LUA_VNUMFLT: {
		lua_Integer k;
		if (luaV_flttointeger(fltvalue(key), &k, F2Ieq)) /* integral
								    index? */
...
lua_Unsigned luaH_getn(Table *t)
{
	...
	if (limit > 0 && isempty(&t->array[limit - 1])) { /* (1)? */
		...
		if (limit >= 2 && !isempty(&t->array[limit - 2])) {
			...
			if (ispow2realasize(t) && !ispow2(limit - 1)) {
				t->alimit = limit - 1;
				setnorealasize(t); /* now 'alimit' is not the
						      real size */
			}
			return limit - 1;
		} else { /* must search for a boundary in [0, limit] */
			unsigned int boundary = binsearch(t->array, 0, limit);
			/* can this boundary represent the real size of the
			 * array? */
			if (ispow2realasize(t)
			    && boundary > luaH_realasize(t) / 2) {
	...
	if (!limitequalsasize(t)) { /* (2)? */
		...
		if (isempty(&t->array[limit - 1])) { /* empty? */
			/* there must be a boundary in the array after old
			   limit, and it must be a valid new limit */
			unsigned int boundary =
				binsearch(t->array, t->alimit, limit);
			t->alimit = boundary;
			return boundary;
		}
	...
	/* (3) 'limit' is the last element and either is zero or present in
	 * table */
