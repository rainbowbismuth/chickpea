		  ...
		  size_t word_count)
{
	...
	register uintptr_t src_r asm("r0") = (uintptr_t)src;
	register uintptr_t dst_r asm("r1") = (uintptr_t)dst;
...
{
	return (struct palette *)(uintptr_t)(BG_PALETTE_RAM
					     + palette_idx * 0x20);
}

struct palette *nonnull obj_palette(uint32_t palette_idx)
{
	return (struct palette *)(uintptr_t)(OBJ_PALETTE_RAM
					     + palette_idx * 0x20);
}

struct char_4bpp *nonnull char_block_begin(uint32_t char_block)
{
	return (struct char_4bpp *)(uintptr_t)(VRAM_BEGIN
					       + char_block * 0x4000);
}

uint16_t *nonnull screen_block_begin(uint32_t screen_block)
{
	return (uint16_t *)(uintptr_t)(VRAM_BEGIN + screen_block * 0x800);
}

volatile uint16_t *nonnull reg_bg_control(enum background bg)
{
	return (volatile uint16_t *)(uintptr_t)(0x04000008 + (bg << 1));
}

void set_bg_scroll_x(enum background bg, uint16_t scroll_x)
{
	*(volatile uint16_t *)(uintptr_t)(0x04000010 + (bg << 2)) = scroll_x;
}

void set_bg_scroll_y(enum background bg, uint16_t scroll_y)
{
	*(volatile uint16_t *)(uintptr_t)(0x04000012 + (bg << 2)) = scroll_y;
}
